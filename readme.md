...
Пример получения диагностики кабеля на коммутаторах SNR-S2985G-24T:
```python
get_CableDiag = {
...
## Пример скрипта для получения результатов диагностики кабеля

В моей компании чаще всего пишут на PHP, поэтому пример будет на этом языке. Сильно не пинайте,
PHP я не помнил и давно забыл.

Сначала нам нужна функция, которая будет отсылать запросы на сервис:

```php
/**
 * Посылает запрос к SWToolz-Core и возвращает результат в виде JSON без каких-либо преобразований.
 *
 * @param $command string Команда, которую нужно послать сервису.
 * @return array Ответ от сервиса в виде JSON.
 */
function swtoolz_send_command(string $command)
{
    $cURL = curl_init("http://swtoolz-url.com/user/$command");
    curl_setopt($cURL, CURLOPT_PORT, 7377);
...
Устанавливаем начальные параметры работы скрипта:

```php
$deviceip = '10.90.90.90'; // ip-адрес коммутатора
$communityId = 3; // id строки community в сервисе SWToolz-Core для обращения к оборудованию
$portnum = 3; // номер порта
```

Выполняем запросы к сервису и отрисовываем результаты:

```php
// Получаем с сервиса расшифровки числовых статусов и запускаем процесс диагностики порта
$statuses = swtoolz_send_command("$deviceip/$communityId/cdPairStatus+/cdLinkStatus+/set_CableDiagInit/$portnum")['response']['data'];
// Получаем с сервиса реузультаты диагностики
$diagResults = swtoolz_send_command("$deviceip/$communityId/get_CableDiag/$portnum")['response']['data'];

$linkStatus = $diagResults['cdLinkStatus'][$portnum]; // статус линка в числовом формате
$humanLinkStatus = $statuses['cdLinkStatus'][$linkStatus]; // статус линка в человекочитаемом формате
print("Статус линка: $humanLinkStatus" . PHP_EOL);

// Как мы знаем, в кабеле 4 пары, "дергаем" параметры для каждой из них
for ($pairNum = 1; $pairNum <= 4; $pairNum++) {
    $pairLength = $diagResults["cdPair{$pairNum}Length"][$portnum]; // длина пары
    $pairStatus = $diagResults["cdPair{$pairNum}Status"][$portnum]; // статус пары в числовом формате
    $humanPairStatus = $statuses['cdPairStatus'][$pairStatus]; // статус пары в человекочитаемом формате
    print("Пара $pairNum: длина - $pairLength, статус - $humanPairStatus" . PHP_EOL);
}
```
