...
# Получение серийных номеров всех ONT за портом.
...
# Получение описаний для всех ONT за портом
# На вход обязательно нужно передать: ifIndex GPON-порта
walk_onts_descriptions_by_port = {
    # description   .1.3.6.1.4.1.2011.6.128.1.1.2.43.1.9  hwGponDeviceOntDespt
...
# Если вместо индекса пользователя передать -1, то OLT сама выберет индекс.
add_igmp_user = [
...
# Получение индекса btv-пользователя по индексу сервисного порта
# На вход обязательно передать: индекс сервисного порта
igmp_user = {
    # igmp_user.   .1.3.6.1.4.1.2011.6.28.106.1.2
    'igmp_user.': '.1.3.6.1.4.1.2011.6.28.106.1.2.{1}',
}
...
# Разрегистрировать ONT.
# На вход обязательно передаем: ifIndex GPON-порта, ont_id.
unregister_ont = [
    # .1.3.6.1.4.1.2011.6.128.1.1.2.43.1.10 hwGponDeviceOntEntryStatus
    ['.1.3.6.1.4.1.2011.6.128.1.1.2.43.1.10', '{1}.{2}', '6', 'INTEGER'],
]
...
# Получение индекса следующего свободного сервискного порта.
next_free_service_port = {
    # next_service_port.   .1.3.6.1.4.1.2011.5.14.5.1.0
    'next_free_service_port.': '.1.3.6.1.4.1.2011.5.14.5.1.0',
}
