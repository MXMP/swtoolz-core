...
def main():
    ...
    while True:
        ...
        try:
            ...
            # Переводим сокет в неблокирующий режим и говорим, чтобы при отключении освобождался быстрее
            # (no linger) и добавляем в список
        ...
        for client_index, client in enumerate(clients):
            ...
            # Пробуем получить IP-адрес и порт текущего клиента. Выше работали с теми, кто передает данные,
            # а дальше работаем со всеми
            try:
                client_ip, client_port = client.getpeername()
            # При неудаче добавляем клиента в список неактивных клиентов, чтобы затем удалить из основного списка,
            # который сейчас перебираем
            except:
            ...
            if answer_immediately:
                ...
                new_header = http_header.replace("{$datetime}", str(
                    datetime.datetime.utcnow().strftime('%a, %d %b %Y %H:%M:%S GMT'))).replace("{$datalen}",
                                                                                               str(len(http_data)))
                if debug_mode:
                ...
                # Сессия поддерживалась бесконечное время при помощи Keep-Alive сообщений. Поэтому
                # отключение производим вручную.
